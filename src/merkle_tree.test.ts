import { MerkleTree, HashPath } from '.';
import { Sha256Hasher } from './sha256_hasher';
import levelup from 'levelup';
import memdown from 'memdown';

describe('merkle_tree', () => {
  const values: Buffer[] = [];

  beforeAll(async () => {
    for (let i = 0; i < 1024; ++i) {
      const v = Buffer.alloc(64, 0);
      v.writeUInt32LE(i, 0);
      values[i] = v;
    }
  });

  it('should have correct empty tree root for depth 32', async () => {
    const db = levelup(memdown());
    const tree = await MerkleTree.new(db, 'test', 32);
    const root = tree.getRoot();
    expect(root.toString('hex')).toEqual('1c9a7e5ff1cf48b4ad1582d3f4e4a1004f3b20d8c5a2b71387a4254ad933ebc5');
  });

  it('should calculate empty tree root efficiently for various depths', async () => {
    // Testing the private calculateEmptyTreeRoot method by creating trees with different depths
    const db = levelup(memdown());
    
    // Test for depth 1
    const tree1 = await MerkleTree.new(db, 'test1', 1);
    expect(tree1.getRoot().toString('hex')).not.toEqual('');
    
    // Test for depth 10
    const tree10 = await MerkleTree.new(db, 'test10', 10);
    expect(tree10.getRoot().toString('hex')).not.toEqual('');
    
    // Test for depth 20
    const tree20 = await MerkleTree.new(db, 'test20', 20);
    expect(tree20.getRoot().toString('hex')).not.toEqual('');
    
    // Test for depth 32 (maximum)
    const tree32 = await MerkleTree.new(db, 'test32', 32);
    expect(tree32.getRoot().toString('hex')).toEqual('1c9a7e5ff1cf48b4ad1582d3f4e4a1004f3b20d8c5a2b71387a4254ad933ebc5');
    
    // Verify the operation is fast (shouldn't take more than a few milliseconds)
    const startTime = process.hrtime();
    await MerkleTree.new(db, 'test_perf', 32);
    const [seconds, nanoseconds] = process.hrtime(startTime);
    
    // Assuming the calculation should take less than 100ms for even depth 32
    // This is a very conservative limit - in practice it should be much faster
    expect(seconds * 1000 + nanoseconds / 1000000).toBeLessThan(100);
  });

  it('should have correct root', async () => {
    const db = levelup(memdown());

    const hasher = new Sha256Hasher();
    const e00 = hasher.hash(values[0]);
    const e01 = hasher.hash(values[1]);
    const e02 = hasher.hash(values[2]);
    const e03 = hasher.hash(values[3]);
    const e10 = hasher.compress(e00, e01);
    const e11 = hasher.compress(e02, e03);
    const root = hasher.compress(e10, e11);

    const tree = await MerkleTree.new(db, 'test', 2);

    for (let i = 0; i < 4; ++i) {
      await tree.updateElement(i, values[i]);
    }

    let expected = new HashPath([
      [e00, e01],
      [e10, e11],
    ]);

    expect(await tree.getHashPath(0)).toEqual(expected);
    expect(await tree.getHashPath(1)).toEqual(expected);

    expected = new HashPath([
      [e02, e03],
      [e10, e11],
    ]);

    expect(await tree.getHashPath(2)).toEqual(expected);
    expect(await tree.getHashPath(3)).toEqual(expected);
    expect(tree.getRoot()).toEqual(root);

    expect(root).toEqual(Buffer.from('e645e6b5445483a358c4d15c1923c616a0e6884906b05c196d341ece93b2de42', 'hex'));
  });

  it('should be able to restore from previous data', async () => {
    const levelDown = memdown();
    const db = levelup(levelDown);
    const tree = await MerkleTree.new(db, 'test', 10);
    for (let i = 0; i < 128; ++i) {
      await tree.updateElement(i, values[i]);
    }

    const db2 = levelup(levelDown);
    const tree2 = await MerkleTree.new(db2, 'test');

    expect(tree.getRoot().toString('hex')).toBe('4b8404d05a963de56f7212fbf8123204b1eb77a4cb16ae3875679a898aaa5daa');
    expect(tree.getRoot()).toEqual(tree2.getRoot());
    for (let i = 0; i < 128; ++i) {
      expect(await tree.getHashPath(i)).toEqual(await tree2.getHashPath(i));
    }
  });

  it('should have correct results inserting 1024 values into 32 depth tree.', async () => {
    console.log('Starting 1024-value test...');
    const startTime = process.hrtime();
    
    const db = levelup(memdown());
    const tree = await MerkleTree.new(db, 'test', 32);

    console.log('Inserting 1024 values...');
    const insertStartTime = process.hrtime();
    for (let i = 0; i < values.length; ++i) {
      await tree.updateElement(i, values[i]);
      if (i % 10 === 0) {
        console.log(`Inserted ${i} values...`);
      }
    }
    const [insertSeconds, insertNanoseconds] = process.hrtime(insertStartTime);
    console.log(`Insertion took ${insertSeconds}s ${insertNanoseconds/1000000}ms`);

    
    console.log('Verifying root hash...');
    const rootHash = tree.getRoot().toString('hex');
    console.log('Root hash:', rootHash);

    expect(rootHash).toBe('26996bfcb0aaf96422aefdd789396a3f6c8a4fa6dccc73e55060e03e2a238db0');

    console.log('Getting hash path...');
    const hashPathStartTime = process.hrtime();
    const hashPath = await tree.getHashPath(100);
    const [hashPathSeconds, hashPathNanoseconds] = process.hrtime(hashPathStartTime);
    console.log(`Hash path calculation took ${hashPathSeconds}s ${hashPathNanoseconds/1000000}ms`);

    expect(hashPath.toBuffer().toString('hex')).toBe(
      '',
    );

    const [totalSeconds, totalNanoseconds] = process.hrtime(startTime);
    console.log(`Total test time: ${totalSeconds}s ${totalNanoseconds/1000000}ms`);
  });
});


describe('levenshteinDistance', () => {
  it('should return 0 for identical strings', () => {
    expect(MerkleTree.levenshteinDistance('hello', 'hello')).toBe(0);
  });

  it('should return length of string when comparing with empty string', () => {
    expect(MerkleTree.levenshteinDistance('', 'hello')).toBe(5);
    expect(MerkleTree.levenshteinDistance('hello', '')).toBe(5);
  });

  it('should handle single character substitutions', () => {
    expect(MerkleTree.levenshteinDistance('cat', 'hat')).toBe(1);
    expect(MerkleTree.levenshteinDistance('kitten', 'sitten')).toBe(1);
  });

  it('should handle insertions and deletions', () => {
    expect(MerkleTree.levenshteinDistance('cat', 'cats')).toBe(1);  // insertion
    expect(MerkleTree.levenshteinDistance('cats', 'cat')).toBe(1);  // deletion
  });

  it('should handle multiple operations', () => {
    expect(MerkleTree.levenshteinDistance('kitten', 'sitting')).toBe(3);  // substitution + insertion + substitution
  });

  it('should handle completely different strings', () => {
    expect(MerkleTree.levenshteinDistance('hello', 'world')).toBe(4);
  });
});